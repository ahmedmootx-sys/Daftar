...
function statusSelectHTML(studentId, sessionId, current, archIdx){
  const sd = state.settings;
...
function renderArchiveDetail(idx){
  ...
  container.innerHTML =
    ...
    + '</div>'
    + '<p class="hint">💡 يمكنك تعديل حالات الحضور والملاحظات والدفع مباشرة هنا — التعديلات تُحفظ في الأرشيف فوراً.</p>'
    + archiveTableHTML(a, idx)
    + '</div>';
  container.scrollIntoView({behavior:'smooth'});
}

function archiveTableHTML(a, idx){
  const sd = state.settings;
  ...
  Array.from(new Set(ids)).forEach(sid => {
    ...
    a.sessions.forEach(s => {
      ...
      h += '<td><div class="cell-wrap">'
         + statusSelectHTML(sid, s.id, rec.status || '', idx)
...
function buildMonthlyReportText(lesson){
  const rows = computeStats(lesson.students, lesson.sessions, lesson.records).slice().sort((a,b) => b.pct - a.pct);
  let text = '📊 تقرير شهر ' + lesson.monthNumber + '/' + lesson.year + ' — ' + lesson.name + '\n';
  text += 'عدد الحصص: ' + lesson.sessions.length + ' · عدد الطلاب: ' + lesson.students.length + '\n';
  text += '\n📈 نسب الحضور:';
  rows.forEach((r, i) => {
  ...
  const events = (lesson.sessions||[]).filter(s => s.event);
  if(events.length){
    text += '\n\n📝 أحداث الشهر:';
...
function monthlyReportMessage(lesson){
  const targets = waTargetOptions();
  const text = buildMonthlyReportText(lesson);
