...
function normalizeState(raw){
  ...
  return { version: 6, settings, moneyPasswordHash: (typeof raw.moneyPasswordHash === 'string') ? raw.moneyPasswordHash : '', lessons, archive };
}
...
let dragState = { dragId: null, touchTimer: null, touchActive: false, ghost: null, startX: 0, startY: 0, row: null };
...
async function revealMoney(){
  if(state.moneyPasswordHash){
    const pwd = await askPassword('🔒 أدخل كلمة السر لرؤية الدخل:');
    if(pwd == null) return;
    const h = await sha256Hex(pwd);
    if(h === state.moneyPasswordHash){ moneyUnlocked = true; renderAll(); }
    else { alert('كلمة السر غير صحيحة.'); }
  } else {
    const pwd = await askPassword('🔑 عيّن كلمة سر جديدة للدخل (أول مرة):');
    if(pwd == null) return;
    if(pwd.length < 4){ alert('استخدم كلمة سر أطول (4 أحرف على الأقل).'); return; }
    state.moneyPasswordHash = await sha256Hex(pwd);
    moneyUnlocked = true;
    saveState(); renderAll();
  }
}
...
function renderGlobalSearch(){
  ...
  state.lessons.forEach(L => {
    L.students.forEach(st => {
      if(normalizeForSearch(st.name).includes(qN) || (st.phone||'').includes(q)){
...
function renderLessonDetail(){
  ...
  if(fq){
    students = students.filter(st => normalizeForSearch(st.name).includes(fqN) || (st.phone||'').includes(fq));
  }
...
function exportLesson(lesson){
  ...
  downloadBlob(JSON.stringify(payload, null, 2), 'درس_' + safe + '_' + lesson.monthNumber + '-' + lesson.year + '.json', 'application/json');
}
...
function weeklyMessage(){
  ...
  const body =
    ...
    + '<div class="form-row"><label>نوع الواتساب<select id="msgType"><option value="normal">واتساب عادي</option><option value="business">واتساب أعمال</option></select></label></div>'
  ...
  link.addEventListener('click', () => {
    const t = buildTarget();
    if(t.isGroup && navigator.clipboard && navigator.clipboard.writeText){ navigator.clipboard.writeText(t.text); }
  });
  update();
...
function initDragReorder(){
  ...
  document.addEventListener('dragover', (e) => {
    const row = e.target.closest('tr.student-row');
    if(!row || !dragState.dragId) return;
    e.preventDefault();
